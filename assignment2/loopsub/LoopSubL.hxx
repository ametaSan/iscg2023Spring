...
class LoopSub {
 public:
  ...
  void setSplit_DS()
  {
      ...
      // odd vertex を作成 (mesh のエッジの数)
      odd_.resize(mesh_->edges().size());
      std::cout << "edges size: " << mesh_->edges().size() << std::endl;
      for (int i = 0; i < mesh_->edges().size(); ++i)
          odd_[i] = submesh_->addVertex(p);

      // face vertex を作成 (mesh のfaceの数)
      face_.resize(mesh_->faces_size());
      std::cout << "edges size: " << mesh_->faces_size() << std::endl;
      for (int i = 0; i < mesh_->faces_size(); ++i)
          face_[i] = submesh_->addVertex(p);

      // submesh の vertex を作成 (#summed up valences)
      nvert_.resize(mesh_->halfedge_size());
      std::cout << "edges size: " << mesh_->halfedge_size() << std::endl;
      for (int i = 0; i < mesh_->halfedge_size(); ++i)
          nvert_[i] = submesh_->addVertex(p);

      // add faces part1
      for (auto fc : mesh_->faces())
      {
          // say the model using DS sub only have quads
          // that is, the #vertices of each face is always 4
          // we can release this assumption here.. 
          // ..anyway, only for simplicity
          ...
          VertexL* e4 = odd_[he->next()->next()->next()->edge()->id()];
          VertexL* f = face_[fc->id()];
          VertexL* nv1 = nvert_[he->id()];
          VertexL* nv2 = nvert_[he->next()->id()];
          VertexL* nv3 = nvert_[he->next()->next()->id()];
          VertexL* nv4 = nvert_[he->next()->next()->next()->id()];
          // std::cout << "he id: " << he->id() << he->next()->id() << std::endl;

          FaceL* fc = submesh_->addFace();
          submesh_->addHalfedge(fc, nv1);
          submesh_->addHalfedge(fc, nv2);
          submesh_->addHalfedge(fc, nv3);
          submesh_->addHalfedge(fc, nv4);
      }

      // add faces part2
      for (auto vt : mesh_->vertices())
      {
          VertexL* nv1 = nvert_[vt->halfedge()->id()];
          VertexL* nv2 = nvert_[vt->halfedge()->mate()->next()->id()];
          VertexL* nv3 = nvert_[vt->halfedge()->mate()->next()->mate()->next()->id()];
          VertexL* nv4 = nvert_[vt->halfedge()->mate()->next()->mate()->next()->mate()->next()->id()];
      ...
      {
          VertexL* rnv1 = nvert_[ed->rhe()->id()];
          VertexL* rnv2 = nvert_[ed->rhe()->next()->id()];
          VertexL* lnv1 = nvert_[ed->lhe()->id()];
          VertexL* lnv2 = nvert_[ed->lhe()->next()->id()];

          FaceL* fc = submesh_->addFace();
          /*submesh_->addHalfedge(fc, rnv1);
          submesh_->addHalfedge(fc, rnv2);
          submesh_->addHalfedge(fc, lnv1);
          submesh_->addHalfedge(fc, lnv2);*/
  ...
  void setStencil_DS()
  {
      ...
      {
          Eigen::Vector3d self_posi = vt->point();
          even_[vt->id()]->setPoint(self_posi);
      }
      ...
      {
          Eigen::Vector3d fp;
          Eigen::Vector3d v1 = even_[fc->begin()->vertex()->id()]->point();
          Eigen::Vector3d v2 = even_[fc->begin()->next()->vertex()->id()]->point();
          Eigen::Vector3d v3 = even_[fc->begin()->next()->next()->vertex()->id()]->point();
          Eigen::Vector3d v4 = even_[fc->begin()->next()->next()->next()->vertex()->id()]->point();
          fp = 0.25f * (v1 + v2 + v3 + v4);
          face_[fc->id()]->setPoint(fp);

          Eigen::Vector3d nv1;
          Eigen::Vector3d ev1 = odd_[fc->begin()->edge()->id()]->point();
          Eigen::Vector3d epv1 = odd_[fc->begin()->prev()->edge()->id()]->point();
          nv1 = 0.25f * (fp + v1 + ev1 + epv1);
          nvert_[fc->begin()->id()]->setPoint(nv1);

          Eigen::Vector3d nv2;
          Eigen::Vector3d ev2 = odd_[fc->begin()->next()->edge()->id()]->point();
          Eigen::Vector3d epv2 = odd_[fc->begin()->next()->prev()->edge()->id()]->point();
          nv2 = 0.25f * (fp + v2 + ev2 + epv2);
          nvert_[fc->begin()->next()->id()]->setPoint(nv2);

          Eigen::Vector3d nv3;
          Eigen::Vector3d ev3 = odd_[fc->begin()->next()->next()->edge()->id()]->point();
          Eigen::Vector3d epv3 = odd_[fc->begin()->next()->next()->prev()->edge()->id()]->point();
          nv3 = 0.25f * (fp + v3 + ev3 + epv3);
          nvert_[fc->begin()->next()->next()->id()]->setPoint(nv3);

          Eigen::Vector3d nv4;
          Eigen::Vector3d ev4 = odd_[fc->begin()->next()->next()->next()->edge()->id()]->point();
          Eigen::Vector3d epv4 = odd_[fc->begin()->next()->next()->next()->prev()->edge()->id()]->point();
          nv4 = 0.25f * (fp + v4 + ev4 + epv4);
          nvert_[fc->begin()->next()->next()->next()->id()]->setPoint(nv4);
      }
  };

  ///////////////////////////////////////////////////student codes ends here
 ...
 private:
  ...
  std::vector<VertexL*> nvert_;   // new vertex, for DS

  // subTag: 0:loop 1:CC 2:DS
  int subTag = 2;   
};
